 ...
 * @generated
 */
public class LogicValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	 ...
	 * @generated
	 */
	private static final String MARKER_TYPE = LogicDiagramEditorPlugin.ID + ".diagnostic"; //$NON-NLS-1$
	...
	public void createDecorators(IDecoratorTarget decoratorTarget) {
		...
		if (editPart instanceof GraphicalEditPart || editPart instanceof AbstractConnectionEditPart) {
			...
			if (((DiagramEditDomain) ed).getEditorPart() instanceof LogicDiagramEditor) {
	...
	public boolean provides(IOperation operation) {
		...
		return view != null && FlowEditPart.MODEL_ID.equals(LogicVisualIDRegistry.getModelID(view));
	}
	...
	private static void refreshDecorators(String viewId, Diagram diagram) {
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			public void run() {
				try {
					TransactionalEditingDomain transactionalEditingDomain = TransactionUtil.getEditingDomain(fdiagram);
					if (transactionalEditingDomain != null) {
						transactionalEditingDomain.runExclusive(new Runnable() {
							public void run() {
					...
					LogicDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
		...
		// public void run() {
		// try {
		// System.out.println(fdiagram+"~~~~~~~~~~~~"+TransactionUtil.getEditingDomain(fdiagram));
		// TransactionUtil.getEditingDomain(fdiagram).runExclusive(new
		// Runnable() {
		//
		// public void run() {
		// for (Iterator it = decorators.iterator(); it.hasNext();) {
	...
	public static class StatusDecorator extends AbstractDecorator {
		...
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				LogicDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
		...
		public void refresh() {
			...
			try {
				...
				LogicDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
	...
	static class MarkerObserver implements IFileObserver {
		...
		private String getType(IMarker marker) {
			try {
				...
				LogicDiagramEditorPlugin.getInstance().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
